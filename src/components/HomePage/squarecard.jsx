...
function Squarecard({
  ...
  const handlewishlist = async () => {
    ...
    let response = await fetch(
      `https://groomerapp-e1f70ad38863.herokuapp.com/user/wishlist/create`,
      {
        method: "Post",
  ...
  return (
    <div>
      <div
        class="cards-container salonspage-totalcard"
      ...
      >
        <div className="cards-positions-container">
          <div id={id} class="carousel slide">
            ...
            <div
              class="carousel-inner salonspage-image"
              style={{ width: "300px" }}
              onClick={(id) => handleonesalon(id)}
            >
              <div class="carousel-item active">
                {/* <img src={imageSrc} class="d-block w-100" alt="..." /> */}
                {
                  <img
                    className="salonspage-imagedisplay"
                    src={img}
                    style={{ height: "370px", width: "276px" }}
                  />
                }
              </div>
          ...
          <span
            onClick={() => navigaettomaps(Location)}
            className="location-icon"
          >
            <i style={{ color: "white" }} class="bi bi-geo-alt-fill"></i>
          </span>
        </div>
        <div className="card-position" onClick={(id) => handleonesalon(id)}>
          <div
            class="card-body salons-display-cardbody"
            style={{ position: "relative", top: "-30px" }}
          >
            <h5
              style={{ color: "white", fontSize: "20px" }}
              class="card-title salons-salonname"
            >
              {content}
            </h5>
            <div style={{ display: "flex" }}>
              ...
              {services.map((data) => (
                <p
                  ...
                  class="card-text salon-salonservices"
                >
                  {data.ServiceName}{" "}
                </p>
              ))}{" "}
            ...
            <p className="salons-price" style={{ color: "white" }}>
              ₹ {price} / person{" "}
              <span onClick={(id) => handleonesalon(id)}>
                {" "}
                <i
                  style={{
                    color: "white",
