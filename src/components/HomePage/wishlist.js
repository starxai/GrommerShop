...
const Wishlist = () => {
    ...
    return (
        <div>
            ...
            <div style={{ marginTop: '20px', textAlign: 'center', marginBottom: '20px',color:"white" }}>
                <h1>Wishlist</h1>
                <form>
                    <label style={{ marginInline: '30px' }}>
                        <input type="radio" value="yes" checked={wishlist === 'yes'} onChange={handleWishlistChange} />
                        Products
                    </label>
                    <label>
                        <input type="radio" value="no" checked={wishlist === 'no'} onChange={handleWishlistChange} />
                        Salons
                    </label>
                </form>
            </div>
                ...
                {wishlistdata.map((data, index) => (
                    <div className='card-box' key={index}>
                        <div>
                            ...
                            <div className='fa-heart'><FontAwesomeIcon icon={faHeart} /></div>
                        </div>
                        <h3 style={{ color: 'white',marginTop:"-45px"}}>{data.name}</h3>

                        <div className='wishlist-services' style={{ color: 'white', fontSize: '13px', marginRight: '10px' }}>{data.services}</div>  
                        <div style={{ color: 'white', marginTop: '15px', fontSize: '15px' }}>Rs {data.price}
                            <div className='fa-arrow'><FontAwesomeIcon icon={faArrowRight} /></div>
                        </div>
                    </div>
                ))}
            </div>
